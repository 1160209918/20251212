...
func (m *WSMonitor) initializeHistoricalData() error {
	...
	for _, symbol := range m.symbols {
		...
		go func(s string) {
			...
			// Get historical K-line data (500 bars to support FRVP lookback=338 + buffer)
			klines, err := apiClient.GetKlines(s, "3m", 500)
			if err != nil {
			...
			// Get historical K-line data (500 bars to support FRVP lookback=338 + buffer)
			klines4h, err := apiClient.GetKlines(s, "4h", 500)
			if err != nil {
				log.Printf("Failed to get %s historical data: %v", s, err)
				return
			}
...
func (m *WSMonitor) processKlineUpdate(symbol string, wsData KlineWSData, _time string) {
	...
	if exists {
		...
		if len(klines) > 0 && klines[len(klines)-1].OpenTime == kline.OpenTime {
			...
			// Maintain data length (500 bars to support FRVP lookback=338 + buffer)
			if len(klines) > 500 {
				klines = klines[1:]
			}
		}
...
func (m *WSMonitor) GetCurrentKlines(symbol string, duration string) ([]Kline, error) {
	...
	if !exists {
		...
		apiClient := NewAPIClient()
		klines, err := apiClient.GetKlines(symbol, duration, 500)
		if err != nil {
