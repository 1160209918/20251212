...
interface FormState {
  ...
  // 反马丁仓位管理参数
  base_capital_ratio?: number
  anti_martingale_profit_conversion?: number
  anti_martingale_usage_ratio?: number
  anti_martingale_pool_limit?: number
}
...
export function TraderConfigModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 包含反马丁仓位管理参数
      if (formData.base_capital_ratio !== undefined) {
        saveData.base_capital_ratio = formData.base_capital_ratio
      }
      if (formData.anti_martingale_profit_conversion !== undefined) {
        saveData.anti_martingale_profit_conversion = formData.anti_martingale_profit_conversion
      }
      if (formData.anti_martingale_usage_ratio !== undefined) {
        saveData.anti_martingale_usage_ratio = formData.anti_martingale_usage_ratio
      }
      if (formData.anti_martingale_pool_limit !== undefined) {
        saveData.anti_martingale_pool_limit = formData.anti_martingale_pool_limit
      }
        ...
        >
          ...
          <div className="bg-[#0B0E11] border border-[#2B3139] rounded-lg p-5">
            <h3 className="text-lg font-semibold text-[#EAECEF] mb-3 flex items-center gap-2">
              <span className="text-[#F0B90B]">4</span> 反马丁仓位管理
            </h3>
            <p className="text-xs text-[#848E9C] mb-4">
              系统将根据这些参数自动计算每笔交易的保证金，无需 AI 计算
            </p>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.base_capital_ratio !== undefined ? formData.base_capital_ratio * 100 : 2}
                    onChange={(e) => {
                      const value = Number(e.target.value) / 100
                      handleInputChange('base_capital_ratio', value)
                    }}
                    className="w-full px-3 py-2 bg-[#0B0E11] border border-[#2B3139] rounded text-[#EAECEF] focus:border-[#F0B90B] focus:outline-none"
                    min="0.1"
                    max="10"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.anti_martingale_profit_conversion !== undefined ? formData.anti_martingale_profit_conversion * 100 : 90}
                    onChange={(e) => {
                      const value = Number(e.target.value) / 100
                      handleInputChange('anti_martingale_profit_conversion', value)
                    }}
                    className="w-full px-3 py-2 bg-[#0B0E11] border border-[#2B3139] rounded text-[#EAECEF] focus:border-[#F0B90B] focus:outline-none"
                    min="0"
                    max="100"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.anti_martingale_usage_ratio !== undefined ? formData.anti_martingale_usage_ratio * 100 : 30}
                    onChange={(e) => {
                      const value = Number(e.target.value) / 100
                      handleInputChange('anti_martingale_usage_ratio', value)
                    }}
                    className="w-full px-3 py-2 bg-[#0B0E11] border border-[#2B3139] rounded text-[#EAECEF] focus:border-[#F0B90B] focus:outline-none"
                    min="0"
                    max="100"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.anti_martingale_pool_limit !== undefined ? formData.anti_martingale_pool_limit * 100 : 50}
                    onChange={(e) => {
                      const value = Number(e.target.value) / 100
                      handleInputChange('anti_martingale_pool_limit', value)
                    }}
                    className="w-full px-3 py-2 bg-[#0B0E11] border border-[#2B3139] rounded text-[#EAECEF] focus:border-[#F0B90B] focus:outline-none"
                    min="0"
                    max="100"
              ...
              <div className="p-3 bg-[#1E2329] border border-[#2B3139] rounded">
                <p className="text-xs text-[#848E9C]">
                  <strong className="text-[#F0B90B]">计算公式：</strong><br />
                  反马丁池 = min(昨日反马丁 + {formData.anti_martingale_profit_conversion !== undefined ? (formData.anti_martingale_profit_conversion * 100).toFixed(0) : 90}% × 盈利 - 亏损, 总资金 × {formData.anti_martingale_pool_limit !== undefined ? (formData.anti_martingale_pool_limit * 100).toFixed(0) : 50}%)<br />
                  基础本金 = 总资金 - 反马丁池<br />
                  单笔保证金 = {formData.base_capital_ratio !== undefined ? (formData.base_capital_ratio * 100).toFixed(1) : 2}% × 基础本金 + {formData.anti_martingale_usage_ratio !== undefined ? (formData.anti_martingale_usage_ratio * 100).toFixed(0) : 30}% × 反马丁池
                </p>
              </div>
            </div>
          </div>
