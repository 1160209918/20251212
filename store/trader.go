...
type Trader struct {
	...
	BaseCapitalRatio               *float64 `json:"base_capital_ratio,omitempty"`                // Base capital usage ratio per trade (default 0.02 = 2%)
	AntiMartingaleProfitConversion *float64 `json:"anti_martingale_profit_conversion,omitempty"` // Profit conversion rate to anti-martingale pool (default 0.9 = 90%)
	AntiMartingaleUsageRatio       *float64 `json:"anti_martingale_usage_ratio,omitempty"`       // Anti-martingale pool usage ratio per trade (default 0.3 = 30%)
	AntiMartingalePoolLimit        *float64 `json:"anti_martingale_pool_limit,omitempty"`        // Anti-martingale pool limit as % of total capital (default 0.5 = 50%)
...
func (s *TraderStore) initTables() error {
	...
	alterQueries := []string{
		...
		// Anti-martingale position management parameters
		`ALTER TABLE traders ADD COLUMN base_capital_ratio REAL`,
		`ALTER TABLE traders ADD COLUMN anti_martingale_profit_conversion REAL`,
		`ALTER TABLE traders ADD COLUMN anti_martingale_usage_ratio REAL`,
		`ALTER TABLE traders ADD COLUMN anti_martingale_pool_limit REAL`,
	}
...
func (s *TraderStore) Create(trader *Trader) error {
	_, err := s.db.Exec(`
		INSERT INTO traders (id, user_id, name, ai_model_id, exchange_id, strategy_id, initial_balance,
		                     ...
		                     base_capital_ratio, anti_martingale_profit_conversion,
		                     anti_martingale_usage_ratio, anti_martingale_pool_limit)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, trader.ID, trader.UserID, trader.Name, trader.AIModelID, trader.ExchangeID, trader.StrategyID,
		...
		trader.BaseCapitalRatio, trader.AntiMartingaleProfitConversion,
		trader.AntiMartingaleUsageRatio, trader.AntiMartingalePoolLimit)
	return err
}
