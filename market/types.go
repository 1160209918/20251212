...
type Data struct {
	...
	CurrentMACD       float64
	CurrentMACDHistogram float64 // MACD Histogram (MACD - Signal)
	CurrentRSI7       float64
	// Supertrend indicator
	Supertrend        float64 // Supertrend line value
	SupertrendDir     int     // Supertrend direction: 1=bullish, -1=bearish
	// FRVP indicator (Fixed Range Volume Profile)
	FRVP              *FRVPData
	OpenInterest      *OIData
	FundingRate       float64
...
// FRVPData Fixed Range Volume Profile data
type FRVPData struct {
	POC float64 `json:"poc"` // Point of Control - price with highest volume
	VAH float64 `json:"vah"` // Value Area High - upper boundary of value area
	VAL float64 `json:"val"` // Value Area Low - lower boundary of value area
}
...
type TimeframeSeriesData struct {
	...
	MACDHistogramValues  []float64  `json:"macd_histogram_values"` // MACD Histogram series
