...
type AutoTraderConfig struct {
	...
	// Anti-martingale position management parameters (trader-level configuration)
	BaseCapitalRatio               *float64 // Base capital usage ratio per trade (default 0.02 = 2%)
	AntiMartingaleProfitConversion *float64 // Profit conversion rate to anti-martingale pool (default 0.9 = 90%)
	AntiMartingaleUsageRatio       *float64 // Anti-martingale pool usage ratio per trade (default 0.3 = 30%)
	AntiMartingalePoolLimit        *float64 // Anti-martingale pool limit as % of total capital (default 0.5 = 50%)
}
...
type AutoTrader struct {
	...
	antiMartingalePool         float64   // Current anti-martingale pool amount
	lastAntiMartingaleUpdate   time.Time // Last anti-martingale pool update time
	yesterdayEquity            float64   // Yesterday's equity (for calculating daily profit/loss)
	antiMartingalePoolMutex    sync.RWMutex // Anti-martingale pool read-write lock
}
...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	return &AutoTrader{
		...
		userID:                userID,
		// Initialize anti-martingale pool
		antiMartingalePool:       0.0,
		lastAntiMartingaleUpdate: time.Now(),
		yesterdayEquity:          config.InitialBalance,
		antiMartingalePoolMutex:  sync.RWMutex{},
	}, nil
}
...
func (at *AutoTrader) buildTradingContext() (*decision.Context, error) {
	...
	// Update anti-martingale pool (daily at UTC 00:00)
	at.updateAntiMartingalePool(totalEquity)

	// Calculate single trade margin based on anti-martingale system
	singleTradeMargin, baseCapital, antiMartingalePool := at.calculateSingleTradeMargin(totalEquity)
	...
	ctx := &decision.Context{
		...
		// Pass anti-martingale calculated values to AI
		SingleTradeMargin:  singleTradeMargin,
		BaseCapital:        baseCapital,
		AntiMartingalePool: antiMartingalePool,
	}
...
// executeAdjustStopLossWithRecord executes adjust stop loss and records detailed information
func (at *AutoTrader) executeAdjustStopLossWithRecord(decision *decision.Decision, actionRecord *store.DecisionAction) error {
	logger.Infof("  🔧 Adjust stop loss: %s", decision.Symbol)

	// Get current positions to find the position side and quantity
	positions, err := at.trader.GetPositions()
	if err != nil {
	...
	if decision.StopLoss <= 0 {
		return fmt.Errorf("❌ Invalid stop loss price: %.4f", decision.StopLoss)
	}

	// Cancel existing stop loss orders
	positionSideUpper := "LONG"
	if positionSide == "short" {
		positionSideUpper = "SHORT"
	}

	if err := at.trader.CancelStopLossOrders(decision.Symbol); err != nil {
		logger.Infof("  ⚠️ Failed to cancel old stop loss orders: %v", err)
		// Continue anyway, as old orders might not exist
	}
	...
	logger.Infof("  ✓ Stop loss updated successfully: %s %s @ %.4f",
		decision.Symbol, positionSide, decision.StopLoss)

	return nil
}

// executeAddPositionWithRecord executes add position and records detailed information
func (at *AutoTrader) executeAddPositionWithRecord(decision *decision.Decision, actionRecord *store.DecisionAction) error {
	logger.Infof("  📈 Add position: %s", decision.Symbol)

	// Get current positions to find the existing position
	positions, err := at.trader.GetPositions()
	if err != nil {
		return fmt.Errorf("failed to get positions: %w", err)
	}
	...
	if !found {
		return fmt.Errorf("❌ No open position found for %s, cannot add to position", decision.Symbol)
	}
	...
	// Calculate add quantity (use position_size_usd if provided, otherwise use a default percentage)
	var addQuantity float64
	if decision.PositionSizeUSD > 0 {
		addQuantity = decision.PositionSizeUSD / marketData.CurrentPrice
	} else {
		// Default: add 10% of current position
		addQuantity = currentQuantity * 0.1
	}

	actionRecord.Quantity = addQuantity
	actionRecord.Price = marketData.CurrentPrice
	actionRecord.Leverage = currentLeverage

	// Execute add position based on direction
	var order map[string]interface{}
	if positionSide == "long" {
		order, err = at.trader.OpenLong(decision.Symbol, addQuantity, currentLeverage)
	} else {
		order, err = at.trader.OpenShort(decision.Symbol, addQuantity, currentLeverage)
	}
	...
	logger.Infof("  ✓ Position added successfully: %s %s, added quantity: %.4f (current: %.4f → new: %.4f)",
		decision.Symbol, positionSide, addQuantity, currentQuantity, currentQuantity+addQuantity)

	return nil
}

// executeReducePositionWithRecord executes reduce position and records detailed information
func (at *AutoTrader) executeReducePositionWithRecord(decision *decision.Decision, actionRecord *store.DecisionAction) error {
	logger.Infof("  📉 Reduce position: %s", decision.Symbol)
	...
	if !found {
		return fmt.Errorf("❌ No open position found for %s, cannot reduce position", decision.Symbol)
	}
	...
	// Calculate reduce quantity (use position_size_usd if provided, otherwise use a default percentage)
	var reduceQuantity float64
	if decision.PositionSizeUSD > 0 {
		reduceQuantity = decision.PositionSizeUSD / marketData.CurrentPrice
	} else {
		// Default: reduce 30% of current position
		reduceQuantity = currentQuantity * 0.3
	}
	...
	// Execute reduce position based on direction
	var order map[string]interface{}
	if positionSide == "long" {
	...
	logger.Infof("  ✓ Position reduced successfully: %s %s, reduced quantity: %.4f (current: %.4f → new: %.4f)",
		decision.Symbol, positionSide, reduceQuantity, currentQuantity, currentQuantity-reduceQuantity)
...
func sortDecisionsByPriority(decisions []decision.Decision) []decision.Decision {
	...
	getActionPriority := func(action string) int {
		...
		case "reduce_position":
			return 2 // Second priority: reduce positions
		case "adjust_stop_loss":
			return 3 // Third priority: adjust stop loss
		case "add_position":
			return 4 // Fourth priority: add to positions
		case "open_long", "open_short":
			return 5 // Fifth priority: open new positions
		case "hold", "wait":
			return 6 // Lowest priority: wait
		default:
...
func (at *AutoTrader) checkPositionDrawdown() {
	...
	for _, pos := range positions {
		...
		// DISABLED: Auto close position on drawdown (user requested to disable this feature)
		/*
		...
		*/
		...
		// Only log drawdown monitoring info (no auto close)
		if currentPnLPct > 5.0 {
			logger.Infof("📊 Drawdown monitoring (disabled): %s %s | Profit: %.2f%% | Peak: %.2f%% | Drawdown: %.2f%%",
				symbol, side, currentPnLPct, peakPnLPct, drawdownPct)
		}
	}
}
...
// updateAntiMartingalePool updates anti-martingale pool daily at UTC 00:00
// Formula: 反马丁池 = min(昨日反马丁 + profit_conversion×盈利 - 亏损, 总资金×pool_limit)
func (at *AutoTrader) updateAntiMartingalePool(currentEquity float64) {
	at.antiMartingalePoolMutex.Lock()
	defer at.antiMartingalePoolMutex.Unlock()

	// Check if it's a new day (UTC 00:00)
	now := time.Now().UTC()
	lastUpdate := at.lastAntiMartingaleUpdate.UTC()

	// If not yet crossed UTC 00:00, no update needed
	if now.Year() == lastUpdate.Year() && now.YearDay() == lastUpdate.YearDay() {
		return
	}

	// Get default parameters if not configured
	profitConversion := 0.9 // Default 90%
	if at.config.AntiMartingaleProfitConversion != nil {
		profitConversion = *at.config.AntiMartingaleProfitConversion
	}

	poolLimit := 0.5 // Default 50%
	if at.config.AntiMartingalePoolLimit != nil {
		poolLimit = *at.config.AntiMartingalePoolLimit
	}

	// Calculate daily profit/loss
	dailyPnL := currentEquity - at.yesterdayEquity

	// Update anti-martingale pool
	// If profit: add profit_conversion% of profit to pool
	// If loss: subtract loss from pool
	var poolChange float64
	if dailyPnL > 0 {
		poolChange = dailyPnL * profitConversion
	} else {
		poolChange = dailyPnL // Negative value, will subtract from pool
	}

	newPool := at.antiMartingalePool + poolChange
	...
	// Apply pool limit (max % of total capital)
	maxPool := currentEquity * poolLimit
	if newPool > maxPool {
		newPool = maxPool
	}

	logger.Infof("💰 Anti-Martingale Pool Update: %.2f → %.2f (Daily P&L: %+.2f, Change: %+.2f, Limit: %.2f)",
		at.antiMartingalePool, newPool, dailyPnL, poolChange, maxPool)

	at.antiMartingalePool = newPool
	at.yesterdayEquity = currentEquity
	at.lastAntiMartingaleUpdate = now
}

// calculateSingleTradeMargin calculates single trade margin based on anti-martingale system
// Formula: 单笔保证金 = base_capital_ratio×基础本金 + usage_ratio×反马丁池
// Where: 基础本金 = 总资金 - 反马丁池
func (at *AutoTrader) calculateSingleTradeMargin(currentEquity float64) (singleTradeMargin, baseCapital, antiMartingalePool float64) {
	at.antiMartingalePoolMutex.RLock()
	defer at.antiMartingalePoolMutex.RUnlock()

	// Get default parameters if not configured
	baseCapitalRatio := 0.02 // Default 2%
	if at.config.BaseCapitalRatio != nil {
		baseCapitalRatio = *at.config.BaseCapitalRatio
	}

	usageRatio := 0.3 // Default 30%
	if at.config.AntiMartingaleUsageRatio != nil {
		usageRatio = *at.config.AntiMartingaleUsageRatio
	}

	// Calculate base capital
	antiMartingalePool = at.antiMartingalePool
	baseCapital = currentEquity - antiMartingalePool

	// Ensure base capital is not negative
	if baseCapital < 0 {
		baseCapital = 0
	}

	// Calculate single trade margin
	singleTradeMargin = baseCapitalRatio*baseCapital + usageRatio*antiMartingalePool

	return singleTradeMargin, baseCapital, antiMartingalePool
}
